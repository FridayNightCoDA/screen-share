function countdownTimer(timerEl) {
  ...
  // Meeting start @ Fri 6:05 PST, or Sat 1:05 UTC
  const START_DAY_UTC = 6;
  const START_HOUR_UTC = 1;
  const START_MIN_UTC = 5;
  ...
  if (
    todayUTC < START_DAY_UTC ||
    todayUTC == START_DAY_UTC && now.getUTCHours() < START_HOUR_UTC ||
    todayUTC == START_DAY_UTC && now.getUTCHours() == START_HOUR_UTC && now.getUTCMinutes() < START_MIN_UTC
  ) {
